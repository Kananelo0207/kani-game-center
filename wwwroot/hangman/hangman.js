(() => 
{
  ...
  function promptNames() {
    const name1 = localStorage.getItem("playerName") || "Kani";
    p1.name = name1;

    const modeText = modeSel.options[modeSel.selectedIndex].text.toLowerCase();
    const modeValue = modeSel.value.toLowerCase();
    
    if (modeValue.includes("cpu") || modeText.includes("cpu")) {
      p2.name = "CPU"; 
      return; // Kills the prompt instantly
    }

    const name2 = (prompt("Enter Player 2 name:", "Player 2") || "").trim() || "Player 2";
    p2.name = name2;
  }
  ...
  function endGame(winnerName){
    ...
    if (p1.name === localStorage.getItem("playerName")) {
        console.log("Syncing Hangman score to the cloud...");
