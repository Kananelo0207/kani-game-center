...
class Position
{
  ...
  translate(dir)
  {
    return new Position(this.row + dir.r, this.col + dir.c);
  }
}
...
class SnakePlayer
{
  ...
  changeDirection(newDir)
  {
    ...
    if (
      (cd === Direction.Up && newDir === Direction.Down) ||
      (cd === Direction.Down && newDir === Direction.Up) ||
      (cd === Direction.Left && newDir === Direction.Right) ||
      (cd === Direction.Right && newDir === Direction.Left)
    ) {
      return;
    }
...
class GameState
{
  ...
  snakeAddFirst(snake, pos)
  {
    snake.positions.unshift(pos);
  }

  snakeRemoveLast(snake)
  {
    snake.positions.pop();
  }
  ...
  markDeadSnake(snake)
  {
    for (const p of snake.positions)
    {
      this.grid[p.row][p.col] = GridValue.DeadSnake;
    }
  }
  ...
  moveSingle(snake)
  {
    ...
    if (
      hit === GridValue.Outside ||
      hit === GridValue.Snake ||
      hit === GridValue.Obstacle ||
      hit === GridValue.DeadSnake
    ) {
      snake.isAlive = false;
...
{
  const size = Math.min(window.innerWidth * 0.94, 720);
...
function resetAll()
{
  ...
  scoreLineEl.textContent = "Press Start or Tap Controls";
  leaderboardEl.textContent = lbText();
  showOverlay("PRESS START OR TAP CONTROLS");
  drawEmpty();
}
...
function drawState(timeMs)
{
  ...
  if (s.positions.length)
  {
    ...
    ctx.fillStyle = "#fff";
    ctx.beginPath();
    ctx.arc(-cell * 0.18, -cell * 0.18, cell * 0.12, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.fillStyle = "#000";
    ctx.beginPath();
    ctx.arc(-cell * 0.18, -cell * 0.18, cell * 0.05, 0, Math.PI * 2);
    ctx.fill();
...
function loop(t)
{
  if (phase !== "playing" || !gameState || !snake) return;
...
async function runTurn(name, startOnLeft)
{
  ...
  // This loop keeps the function "alive" while you are playing
  while (snake.isAlive)
  {
    await delay(50);
  }
  ...
  {
      console.log(`Sending ${snake.score} points to the C# Backend...`);
      // Added 'await' so the game waits for the server's "OK"
      await submitSnakeScore(snake.score); 
      showOverlay("SCORE SAVED! TAP TO CONTINUE");
  }
  else 
  {
      showOverlay("LOCAL MATCH FINISHED. TAP TO CONTINUE");
  }
...
async function runGame() 
{
  ...
  // AUTO-BRIDGE: No more prompts for Kani
  const p1 = localStorage.getItem("playerName") || "Kani";
  
  // Only ask for a friend
...
function setSnakeDirectionFromInput(dir)
{
  ...
  if (dir === "left") snake.changeDirection(Direction.Left);
  if (dir === "right") snake.changeDirection(Direction.Right);
  if (dir === "up") snake.changeDirection(Direction.Up);
  if (dir === "down") snake.changeDirection(Direction.Down);
}
...
window.addEventListener("keydown", (e) =>
{
  ...
  if (e.key === "ArrowDown") setSnakeDirectionFromInput("down");
});
...
canvas.addEventListener("touchend", (e) =>
{
  ...
  {
    setSnakeDirectionFromInput(dy > 0 ? "down" : "up");
  }
}, { passive: true });
